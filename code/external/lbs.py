...
def lbs(
    ...
    # v_shaped_ori = ori_v_template + blend_shapes(betas, shapedirs)
    # J = vertices2joints(J_regressor, v_shaped_ori)
    ...
    return (
        ...
        T,
        W,
        A.view(batch_size, num_joints, 4, 4),
        v_posed.clone(),
        pose_feature
    )
...
def inverse_pts(pnts_p, betas, transformations, pose_feature, shapedirs, posedirs, lbs_weights, dtype=torch.float32):
    ...
    pose_offsets = pose_correctives(pose_feature, posedirs).unsqueeze(0)
...
def pose_correctives(pose_feature, posedirs):
    ...
    # pose_correctives = torch.einsum('bi,mik->bmk', [pose_feature, posedirs])
