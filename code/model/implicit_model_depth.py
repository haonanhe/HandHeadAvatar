...
class HHAvatar(nn.Module):
    def __init__(self, conf, shape_params, gt_w_seg):
        ...
        self.flame_faces = self.FLAMEServer.faces_tensor.detach().cpu().numpy().astype(np.int64)
    ...
    def query_sdf_flame(self, pnts_p, verts=None):
        
        def pysdf_func(pnts_p, verts=verts):
            verts = verts.reshape(-1, 3)
            pnts_p = pnts_p.reshape(-1, 3)
            f = SDF(verts.squeeze(0).detach().cpu().numpy(), self.flame_faces)
            flame_sdf = f(pnts_p.detach().cpu().numpy())
            flame_sdf = torch.from_numpy(flame_sdf).to(pnts_p.device)#.unsqueeze(-1)
            flame_sdf = flame_sdf * -1
            return flame_sdf

        flame_sdf = pysdf_func(pnts_p, verts=verts)
        
        sdf = flame_sdf.reshape(-1,)
    ...
    def forward(self, input, return_sdf=False):
        ...
        if "depth" in input:
            if (not 'depth_scale' in input) or input['depth_scale'] is None:
                with torch.no_grad():
                    sdf_function_flame = lambda x: self.query_sdf_flame(pnts_p=x, 
                                                                        verts=verts_head,
                                                                        )[0]
                    points_flame, pred_flame_mask, dists_flame = self.ray_tracer.forward(sdf=sdf_function_flame,
                                                                                            cam_loc=cam_loc,
                                                                                            object_mask=head_mask,
                                                                                            ray_directions=ray_dirs)

                points_flame = (cam_loc.unsqueeze(1) + dists_flame.reshape(batch_size, num_pixels, 1) * ray_dirs).reshape(-1, 3)
                depth_values_flame = torch.zeros_like(points_flame[:, 0]).float().cuda()
                depth_values_flame[pred_flame_mask] = rend_util.get_depth(points_flame.reshape(batch_size, -1, 3), pose).reshape(-1)[pred_flame_mask]
                depth_scale, depth_shift = compute_scale_and_shift(input["depth"][:, :, None], depth_values_flame[None, :, None], (head_mask & surface_mask & pred_flame_mask)[None, :, None])
                depth_scale = depth_scale.item()
                depth_shift = depth_shift.item()
            else:
                depth_scale = input['depth_scale']
                depth_shift = input['depth_shift']
            depth = input["depth"][0] * depth_scale + depth_shift
        ...
        output = {
            ...
            'depth': depth,
            # 'depth_values_flame': depth_values_flame
        }
        if 'depth' in input:
        ...
        if self.training and optimize_contact:
            ...
            samples_hand = sample_points_from_meshes(meshes_hand, num_samples=3000, return_normals=False, return_textures=False)
            ...
            samples_head = sample_points_from_meshes(meshes_head, num_samples=3000, return_normals=False, return_textures=False)
