...
from flame.lbs import inverse_pts
...
class BackwardDeformer(nn.Module):
    ...
    def forward(self, points, conditions, pose_feature, betas, transformations, transl=None):
...
#         num_pixels = int(points.shape[0] / nonrigid_params.shape[0])
#         nonrigid_params = nonrigid_params.unsqueeze(1).expand(-1, num_pixels, -1).reshape(-1, 30)
#         x = torch.cat([points, nonrigid_params], dim=1)
...
#         nonrigid_deformation = self.lin_out(x).reshape(-1, 3)
#         nonrigid_dir = nonrigid_params.unsqueeze(1).repeat(1, 3, 1)
