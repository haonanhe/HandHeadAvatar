...
class FLAME(nn.Module):
    ...
    def __init__(self, flame_model_path, n_shape, n_exp, shape_params):
        ...
        nonrigid_dir = np.load(os.path.join(os.path.dirname(flame_model_path), 'pca_basis.npy'))
        nonrigid_dir = to_tensor(to_np(nonrigid_dir), dtype=self.dtype)
        nonrigid_dir = nonrigid_dir.reshape(-1, 5023, 3).permute(1, 2, 0) # (5023, 3, 30)
        self.register_buffer('nonrigid_dir', nonrigid_dir * factor)
    ...
    # # FLAME mesh morphing
    # def forward(self, expression_params, full_pose, scale=None, transl=None):
    #     """
    #         Input:
    ...
    #     if scale is not None and transl is not None:
    #         # template_vertices = template_vertices * scale
    #         # vertices, pose_feature, transformations = lbs(betas, full_pose, template_vertices,
    #         #               self.shapedirs * scale, self.posedirs * scale,
    ...
    #         # # ### 
    #         # transformations[:, :, :3, :] = transformations[:, :, :3, :].clone() * scale#.reshape(-1, 1, 1, 1)
    ...
    #                         self.lbs_weights, dtype=self.dtype)
    #         vertices = vertices * scale
    #         # transformations[:,:,:3,3] = transformations[:,:,:3,3].clone() + transl.reshape(-1, 1, 1, 3)
    ...
    #         #     [0, 1,  0],
    #         #     [0,  0, 1],
    #         # ], dtype=torch.float32)[None, None, :, :].repeat(1, transformations.shape[1], 1, 1).to(transformations.device)
    #         # transformations[:, :, :3, :3] = torch.matmul(S, transformations[:, :, :3, :3])
    #         # transformations[:, :, :3, 3:] = torch.matmul(S, transformations[:, :, :3, 3:])
    #     else:
    #         vertices, pose_feature, transformations = lbs(betas, full_pose, template_vertices,
    #                         self.shapedirs, self.posedirs,
