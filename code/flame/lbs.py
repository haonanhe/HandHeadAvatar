...
def forward_skinning_pts(pnts_c, transformations, lbs_weights, dtype=torch.float32, mask=None):
    ...
    try:
        ...
        print("w, tran: ", W.shape, transformations.shape)
        exit()
...
def inverse_skinning_pts(pnts_p, transformations, lbs_weights, dtype=torch.float32):
    ...
    try:
        ...
        # dirty fix, better to install newer version of pytorch
        # https://github.com/facebookresearch/pytorch3d/issues/1399
        v_homo = torch.matmul(torch.inverse(T.cpu()).cuda(), torch.unsqueeze(pnts_p, dim=-1))
    # pnts: [num_points, 3]
    pnts = v_homo[:, :3, 0]
...
def blend_shapes(betas, shape_disps):
    ...
    if len(shape_disps.shape) == 3:
        try:
            ...
            print("beta, shape: ", betas.shape, shape_disps.shape)
            exit()
    return blend_shape
...
def pose_correctives(pose_feature, posedirs):
    ...
    try:
        ...
        print("pose_feature, posedir: ", pose_feature.shape, posedirs.shape)
