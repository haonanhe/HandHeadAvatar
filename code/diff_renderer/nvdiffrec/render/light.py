...
class EnvironmentLight(torch.nn.Module):
    ...
    def shade_pbr_ipe(self, gb_pos, diffuse, enc_func, shading_net, gb_normal, kd, kr, view_pos, ko, canon_norm, fresnel_constant, shadow_mlp=None, fourier_feature_transform=None):
        ...
        # # reflvec = (reflvec + 1) / 2
        # enc_reflvec = fourier_feature_transform((reflvec).view(-1, 3)).to(torch.float32)
        # vis_maps = shadow_mlp(enc_reflvec) 
        # vis_maps = vis_maps - vis_maps.mean()
        # vis_maps = torch.sigmoid(vis_maps * 3.0)
        # vis_maps = vis_maps.view(bz, h, w, 1) 
        
        # if ko is not None:
        #     shaded_col *= vis_maps
        #     shaded_col += vis_maps * ko * spec * reflectance
        # else:
        #     shaded_col *= vis_maps
        #     shaded_col += vis_maps * spec * reflectance
