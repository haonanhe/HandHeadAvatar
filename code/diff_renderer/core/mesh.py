...
class Mesh:
    ...
    def __init__(self, vertices, indices, device='cpu'):
        ...
        #     self.color = color.to(device, dtype=torch.float32) if torch.is_tensor(color) else torch.tensor(color, dtype=torch.float32, device=device)
        # else:
        #     self.color = color
    ...
    def to(self, device):
        ...
        #     mesh = Mesh(self.vertices.to(device), self.indices.to(device), self.color.to(device), device=device)
        # else:
    ...
    def detach(self):
        # if self.color is not None:
        #     mesh = Mesh(self.vertices.detach(), self.indices.detach(), self.color.detach(), device=self.device)
        # else:
    ...
    def with_vertices(self, vertices):
        ...
        # mesh_new = Mesh(vertices, self.indices, self.color, self.device)
    ...
    def edges(self):
        if self._edges is None:
            from .geometry import find_edges
    ...
    def connected_faces(self):
        if self._connected_faces is None:
            from .geometry import find_connected_faces
    ...
    def laplacian(self):
        if self._laplacian is None:
            from .geometry import compute_laplacian_uniform
