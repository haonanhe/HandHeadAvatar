...
def plot(img_index, sdf_function, sdf_function_left_hand, sdf_function_right_hand, model_outputs, pose, ground_truth, path, epoch, img_res, plot_nimgs, min_depth, max_depth, res_init, res_up, is_eval=False):
    ...
    # depth_values = rend_util.get_depth(points, pose).reshape(-1)[network_object_mask]
    # try:
    ...
    # depth[network_object_mask] = depth_values
    ...
    #     depth_values_hand = rend_util.get_depth(points_hand, pose).reshape(-1)[network_object_mask_hand]
    ...
    #     depth_hand[network_object_mask_hand] = depth_values_hand
    ...
    #     mask_depth = network_object_mask | network_object_mask_hand
    ...
    plot_images(model_outputs, depth, depth_hand, ground_truth, path, epoch, img_index, 1, img_res, batch_size, num_samples, is_eval)
    # del depth, points, network_object_mask
    del points, network_object_mask
    # Generate mesh.
    if True: # is_eval:
        with torch.no_grad():
            import time
            ...
            meshexport = mesh_util.generate_mesh(sdf_function, verts=model_outputs['verts'].reshape(-1, 3), level_set=0, res_init=res_init, res_up=res_up)
            ...
            #     meshexport = mesh_util.generate_mesh(sdf_function_hand, level_set=0, res_init=res_init, res_up=res_up)
            #     meshexport.export('{0}/surface_{1}_hand.ply'.format(path, img_index), 'ply')
            
            if sdf_function_left_hand is not None: 
                # meshexport = mesh_util.generate_mesh(sdf_function_left_hand, level_set=0, res_init=res_init, res_up=res_up)
                meshexport = mesh_util.generate_mesh(sdf_function_left_hand, verts=model_outputs['verts_hand_left'].reshape(-1, 3), level_set=0, res_init=res_init, res_up=res_up)
                meshexport.export('{0}/surface_{1}_left_hand.ply'.format(path, img_index), 'ply')
                
            if sdf_function_right_hand is not None: 
                meshexport = mesh_util.generate_mesh(sdf_function_right_hand, verts=model_outputs['verts_hand_right'].reshape(-1, 3), level_set=0, res_init=res_init, res_up=res_up)
                meshexport.export('{0}/surface_{1}_right_hand.ply'.format(path, img_index), 'ply')
...
def plot_images(model_outputs, depth_image, depth_image_hand, ground_truth, path, epoch, img_index, plot_nrow, img_res, batch_size, num_samples, is_eval):
    if 'rgb' in ground_truth:
        # rgb_gt = ground_truth['rgb']
        # # rgb_gt = rgb_gt * model_outputs['head_mask'][None,:,None]
        # rgb_gt = rgb_gt * model_outputs['object_mask'][None,:,None]
    ...
    # rgb_points = torch.ones(batch_size * num_samples, 3).cuda().float()
    # rgb_points[model_outputs['network_object_mask']] = model_outputs['rgb_values'][model_outputs['network_object_mask']]
    # rgb_points = rgb_points.reshape(batch_size, num_samples, 3)

    normal_points = torch.ones(batch_size * num_samples, 3).cuda().float()
    normal_points[model_outputs['network_object_mask']] = model_outputs['normal_values'][model_outputs['network_object_mask']]
    ...
    output_vs_gt = normal_points
    
    # if rgb_gt is not None:
    #     output_vs_gt = torch.cat((output_vs_gt, rgb_gt, depth_image[None,:,None].repeat(1, 1, 3), depth_image_hand[None,:,None].repeat(1, 1, 3)), dim=0)
    #     # output_vs_gt = torch.cat((output_vs_gt, rgb_gt, mask_gt.repeat(1, 1, 3), depth_image.repeat(1, 1, 3), normal_points), dim=0)
    # else:
    #     output_vs_gt = torch.cat((output_vs_gt, depth_image[None,:,None].repeat(1, 1, 3)), dim=0)

    # if depth_gt is not None:
    #     output_vs_gt = torch.cat((output_vs_gt, depth_gt.repeat(1, 1, 3)), dim=0)
        
    if 'normal_values_hand' in model_outputs:
        # rgb_points_hand = torch.ones(batch_size * num_samples, 3).cuda().float()
        # rgb_points_hand[model_outputs['network_object_mask_hand']] = model_outputs['rgb_values_hand'][model_outputs['network_object_mask_hand']]
        # rgb_points_hand = rgb_points_hand.reshape(batch_size, -1, 3)
        # rgb_points_hand = (rgb_points_hand + 1.) / 2.
        # # output_vs_gt = torch.cat((output_vs_gt, rgb_points_hand), dim=0)
        
        normal_points_hand = torch.ones(batch_size * num_samples, 3).cuda().float()
        normal_points_hand[model_outputs['network_object_mask_hand']] = model_outputs['normal_values_hand'][model_outputs['network_object_mask_hand']]
        normal_points_hand = normal_points_hand.reshape(batch_size, -1, 3)
        normal_points_hand = (normal_points_hand + 1.) / 2.
        # output_vs_gt = torch.cat((output_vs_gt, normal_points_hand), dim=0)
        ...
        # rgb_points_all[depth_head_mask] = rgb_points.reshape(-1,3)[depth_head_mask]
        ...
        normal_points_all[depth_head_mask] = normal_points.reshape(-1,3)[depth_head_mask]
        ...
        # output_vs_gt = torch.cat((rgb_points_hand, output_vs_gt, rgb_points_all, rgb_gt, depth_image[None,:,None].repeat(1, 1, 3), depth_gt.repeat(1, 1, 3), normal_points_hand, normal_points, normal_points_all), dim=0)
        output_vs_gt = torch.cat((rgb_gt, output_vs_gt, normal_points_all), dim=0)
    ...
    #     lbs_points = model_outputs['lbs_weight']
    #     lbs_points = lbs_points.reshape(batch_size, num_samples, -1)
    #     lbs_points = weights2colors(lbs_points[:, :, :, None])
    #     mask = torch.logical_not(model_outputs['network_object_mask'])
    #     lbs_points[mask[None, ..., None].expand(-1, -1, 3)] = 1.
    #     output_vs_gt = torch.cat((output_vs_gt, lbs_points), dim=0)
    ...
    # if 'deform_dir' in model_outputs:
    #     deform_dir_points = model_outputs['deform_dir']
    ...
    #     gt_deform_dir_points = model_outputs['gt_deform_dir'].clone()
