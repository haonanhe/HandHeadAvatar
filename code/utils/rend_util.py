...
def load_depth_metahuman(path, img_res, scale, depth_scale=1.):
    try:
        depths_np = cv2.imread(path, cv2.IMREAD_UNCHANGED) 
        depths_np = depths_np / depth_scale
    except:
        breakpoint()
    depths_np = depths_np * scale
    depths_np = cv2.resize(depths_np, (int(img_res[0]), int(img_res[1])))
...
def get_camera_params(uv, pose, intrinsics):
    ...
    world_coords = torch.bmm(p, pixel_points_cam).permute(0, 2, 1)[:, :, :3]#.contiguous()
...
def get_depth(points, pose):
    ...
    try:
        ...
        points_cam = torch.inverse(pose.cpu()).cuda().bmm(points_hom)
