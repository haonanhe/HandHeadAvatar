...
class TestRunner():
    def __init__(self, **kwargs):
        ...
        if self.optimize_pca:
            ...
            data = torch.load(
                os.path.join(old_checkpnts_dir, self.input_params_subdir, str(kwargs['checkpoint']) + ".pth"))
            self.deform_params = torch.nn.Embedding(data["deform_params_state_dict"]['weight'].shape[0], 30, sparse=True).cuda()
    ...
    def run(self):
        ...
        for img_index in range(len(self.plot_dataset)):
            ...
            # if model_input["img_name"].item() != 1560:
            #     continue
            ...
            # if self.optimize_contact:
            #     model_input['latent_codes_nonrigid'] = self.latent_codes_nonrigid(model_input["idx"]*self.subsample).squeeze(1).detach()
            if self.optimize_expression:
                model_input['expression'] = self.expression(model_input["idx"]*self.subsample).squeeze(1).detach()
            if self.optimize_pca:
                model_input['deform_params'] = self.deform_params(model_input["idx"]*self.subsample).squeeze(1).detach()
            if self.optimize_pose:
            ...
            # batch_size = model_input['mano_hand_pose'].shape[0]
