...
class Pytorch3dRasterizer(nn.Module):
    ...
    def forward(self, vertices, faces, attributes=None, h=None, w=None, render_mano=False):
        ...
        # # mask mano bottom
        # if render_mano:
        #     F = faces.shape[1]  # Faces per mesh (assumed same for all batches)
        #     original_face_idx = pix_to_face.clone() % F  # For valid faces, gives 0 <= idx < F
        #     mask = (original_face_idx > 1538) & (pix_to_face != -1)
        #     pix_to_face = pix_to_face.clone()
        #     pix_to_face[mask] = -1  # Set to background
...
class SRenderY(nn.Module):
    ...
    def render_shape(self, vertices, transformed_vertices, colors = None, images=None, detail_normal_images=None, 
        ...
        # meshes = Meshes(verts=vertices, faces=self.faces.expand(batch_size, -1, -1))
        ...
        #TODO
        alpha_images_expanded = alpha_images.expand(-1, 3, -1, -1) 
        normal_images = torch.where(alpha_images_expanded == 0, torch.ones_like(normal_images), normal_images)
    ...
    #     # save_ply('./verts_p3d.ply', transformed_vertices[0])
    ...
    def render_depth(self, transformed_vertices, transformed_vertices_cam, render_mano=False):
        '''
        -- rendering depth
        '''
        batch_size = transformed_vertices.shape[0]

        # z = transformed_vertices_cam[:,:,2:3].repeat(1,1,3).clone()
        z = transformed_vertices[:,:,2:].repeat(1,1,3).clone()
        
        attributes = util.face_vertices(z, self.faces.expand(batch_size, -1, -1))

        rendering = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes, render_mano=render_mano)
    ...
    def render_neural_colors(self, neural_shader, vertices, transformed_vertices):
        batch_size = transformed_vertices.shape[0]
        transformed_face_vertices = util.face_vertices(transformed_vertices, self.faces.expand(batch_size, -1, -1))
        attributes = transformed_face_vertices
        position = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes)[:, :3, :, :].permute(0,2,3,1)
        ...
        face_normals = util.face_vertices(normals, self.faces.expand(batch_size, -1, -1))
        attributes = face_normals
        normal = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes)[:, :3, :, :].permute(0,2,3,1)
        
        transformed_normals = util.vertex_normals(transformed_vertices, self.faces.expand(batch_size, -1, -1))
        
        # # swap x and z axises
        # tmp = transformed_normals[:,:,0].clone()
        # transformed_normals[:,:,0] = transformed_normals[:,:,2]
        # transformed_normals[:,:,2] = tmp
        
        transformed_face_normals = util.face_vertices(transformed_normals, self.faces.expand(batch_size, -1, -1))
        attributes = transformed_face_normals
        normal_bend = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes)[:, :3, :, :].permute(0,2,3,1)
        
        # return normal, normal_bend

        pred_color = neural_shader(position, normal_bend, skin_mask=None)

        return pred_color, normal