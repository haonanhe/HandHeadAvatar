...
class Optimizer(object):
    ...
    def optimize(self, shape, exp, landmark, pose, name, visualize_images, savefolder, intrinsics, json_path, size,
        ...
        # if GLOBAL_POSE:
        #     translation_p = torch.tensor([0, 0, 0]).float().cuda()
        # else:
        #     translation_p = torch.tensor([0, 0, 0]).unsqueeze(0).expand(num_img, -1).float().cuda()
        translation_p = torch.tensor([0, 0, 0]).unsqueeze(0).expand(num_img, -1).float().cuda()
        # if GLOBAL_POSE:
        #     translation_c = torch.tensor([0, 0, -4]).float().cuda()
        # else:
        #     translation_c = torch.tensor([0, 0, -4]).unsqueeze(0).expand(num_img, -1).float().cuda()
        ...
        if json_path is None:
            # translation_c = torch.tensor([0, 0, -4]).float().cuda()
            translation_c = torch.tensor([0, 0, -1]).float().cuda()
            # translation_c = torch.tensor([0, 0, -4]).unsqueeze(0).expand(num_img, -1).float().cuda()
            translation_c = nn.Parameter(translation_c)
        
        # set optimizer
        if json_path is None:
            opt_p = torch.optim.Adam(
                [translation_c, translation_p, pose, exp, shape],
                lr=1e-2)
        else:
            opt_p = torch.optim.Adam(
        ...
        for k in range(1001):
            ...
            verts_p += translation_p.unsqueeze(1)
            landmarks3d_p += translation_p.unsqueeze(1)
            landmarks2d_p += translation_p.unsqueeze(1)
            ...
            if json_path is None:
                if GLOBAL_POSE:
                    w2c_p = torch.cat([ident, translation_c.unsqueeze(0).expand(num_img, -1).unsqueeze(2)], dim=2)
                else:
                    w2c_p = torch.cat([ident, translation_c.unsqueeze(2)], dim=2)
            else:
                w2c_p = torch.tensor(json.load(open(json_path, 'r'))['frames'][0]['world_mat']).cuda().unsqueeze(0).expand(num_img, -1, -1)
            # w2c_p = torch.eye(4).float().cuda().unsqueeze(0).expand(num_img, -1, -1)#.unsqueeze(2)
                    ...
                    if save:
                        ...
                        for i in range(num_img):
                            frames.append({'file_path': './image/' + name[i],
                                           ...
                                           'translation_p': translation_p[i].detach().cpu().numpy().tolist(),
                                           })

                        dict['frames'] = frames
                        dict['intrinsics'] = save_intrinsics
                        dict['shape_params'] = shape[0].cpu().numpy().tolist()
                        # dict['translation_c'] = translation_c.cpu().numpy().tolist()
                        with open(os.path.join(savefolder, save_name + '.json'), 'w') as fp:
                            json.dump(dict, fp)
                        
                        np.save(os.path.join(savefolder, 'posed_verts.npy'), verts_p.detach().cpu().numpy())
    ...
    def run(self, deca_code_file, face_kpts_file, iris_file, savefolder, image_path, json_path, intrinsics, size,
        ...
        for k in range(1, num_img + 1):
            ...
            if k % 50 == 1:
                ...
                # image = cv2.imread(image_path + '/%07d.png'%k).astype(np.float32) / 255.
                image = image[:, :, [2, 1, 0]].transpose(2, 0, 1)
                visualize_images.append(torch.from_numpy(image[None, :, :, :]).cuda())
        ...
        #     # translation_c = torch.tensor([0, 0, -4]).unsqueeze(0).expand(num_img, -1).float().cuda()
        # else:
        #     translation_c = torch.tensor(json.load(open(json_path, 'r'))['translation_c']).cuda()#.unsqueeze(0).expand(num_img, -1)
