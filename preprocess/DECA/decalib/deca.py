...
class DECA(nn.Module):
    def __init__(self, config=None, image_size=224, uv_size=224, device='cuda', subject='Yuri'):
        ...
        self._setup_renderer(self.cfg.model, uv_size, subject=subject)

    def _setup_renderer(self, model_cfg, image_size=224, uv_size=224, subject='Yuri'):
        set_rasterizer(self.cfg.rasterizer_type)
        # self.render = SRenderY(self.image_size, obj_filename=model_cfg.topology_path, uv_size=uv_size, rasterizer_type=self.cfg.rasterizer_type).to(self.device)
        self.render = SRenderY(image_size, obj_filename=model_cfg.topology_path, uv_size=uv_size, rasterizer_type=self.cfg.rasterizer_type).to(self.device)
        ...
        mano_obj = '../preprocess/submodules/DECA/decalib/assets/cano_uv_closed.obj'
        if os.path.exists(mano_obj):
            self.render_hand = SRenderY(image_size, obj_filename=mano_obj, uv_size=uv_size, rasterizer_type=self.cfg.rasterizer_type).to(self.device)
        
        hand_head_obj = '../preprocess/submodules/DECA/decalib/assets/hand_head_uv_template_closed.obj'
        if os.path.exists(hand_head_obj):
            self.render_hand_head = SRenderY(image_size, obj_filename=hand_head_obj, uv_size=uv_size, rasterizer_type=self.cfg.rasterizer_type).to(self.device)    

        
        # ###########################
        # head_2hands_obj = '../preprocess/submodules/DECA/decalib/assets/head_2hands_uv_template_closed.obj'
        # self.render_2hands_head = SRenderY(image_size, obj_filename=head_2hands_obj, uv_size=uv_size, rasterizer_type=self.cfg.rasterizer_type).to(self.device)
        
        # hands2_obj = '../preprocess/submodules/DECA/decalib/assets/hands2_uv_template_closed.obj'
        # self.render_2hands = SRenderY(image_size, obj_filename=hands2_obj, uv_size=uv_size, rasterizer_type=self.cfg.rasterizer_type).to(self.device)
        
        # mano_obj = '../preprocess/submodules/DECA/decalib/assets/MANO_UV_left.obj'
        # self.render_hand_left = SRenderY(image_size, obj_filename=mano_obj, uv_size=uv_size, rasterizer_type=self.cfg.rasterizer_type).to(self.device)
        # ###########################
        
        # Metahuman_head_obj = '../preprocess/submodules/DECA/decalib/assets/meatahuman/' + subject + '_head.obj'
        # self.render_Metahuman_head = SRenderY(image_size, obj_filename=Metahuman_head_obj, uv_size=uv_size, rasterizer_type=self.cfg.rasterizer_type).to(self.device)
        
        # Metahuman_hand_obj = '../preprocess/submodules/DECA/decalib/assets/meatahuman/' + subject + '_hand.obj'
        # self.render_Metahuman_hand = SRenderY(image_size, obj_filename=Metahuman_hand_obj, uv_size=uv_size, rasterizer_type=self.cfg.rasterizer_type).to(self.device)
        
        # Metahuman_hand_head_obj = '../preprocess/submodules/DECA/decalib/assets/meatahuman/' + subject + '_hand_head.obj'
        # self.render_Metahuman= SRenderY(image_size, obj_filename=Metahuman_hand_head_obj, uv_size=uv_size, rasterizer_type=self.cfg.rasterizer_type).to(self.device)

        # Emory_hand_head_obj = '/home/haonan/data/NDR_datasets/Emory_aroundorigin/meshes_blender/000000.obj'
        # self.render_Emory = SRenderY(image_size, obj_filename=Emory_hand_head_obj, uv_size=uv_size, rasterizer_type=self.cfg.rasterizer_type).to(self.device)
