...
def l2_distance_v2(verts1, verts2, weight=None):
    # if verts1.shape[1] == 21:
    #     # finger_tips = [4,8,12,16,20]
    #     # index_finger = [5,6,7,8]
    #     index_finger = [8] # index finger tip
...
class Optimizer(object):
    def __init__(self, device='cuda:0', save_folder=None):
    ...
    def optimize(self, shape, exp, face_landmarks, face_poses, betas, global_orient, hand_landmarks, hand_poses, name, visualize_images, savefolder, intrinsics, json_path, size,
        ...
        contact_idx =contact_idx[19:] #fingers
        ...
        # pose_ori = pose.clone()
        hand_poses_ori = hand_poses.clone()
        global_orient_ori = global_orient.clone()
       ...
       ################## Stage I ##################
        ...
        opt_t = torch.optim.Adam(
            [translation_v, translation_f, mano_scale, flame_scale, face_poses],
            lr=1e-2)
        ...
        # for k in range(101):
            ...
            trans_landmarks2d = torch.cat([trans_landmarks2d_hand, trans_landmarks2d_face], dim=1)
            ...
            total_loss += torch.mean(torch.square(face_poses[1:] - face_poses[:-1]))
            total_loss += torch.mean(torch.square(global_orient[1:] - global_orient[:-1])) #* 1e-1
            total_loss += torch.mean(torch.square(hand_poses[1:] - hand_poses[:-1])) 
            total_loss += torch.mean(torch.square(translation_f[1:] - translation_f[:-1])) #* 10
            total_loss += torch.mean(torch.square(translation_v[1:] - translation_v[:-1])) #* 10
            total_loss += torch.mean(torch.square(trans_landmarks3d_face[1:] - trans_landmarks3d_face[:-1])) * 100
            total_loss += torch.mean(torch.square(trans_landmarks3d_hand[1:] - trans_landmarks3d_hand[:-1])) * 100
            total_loss += torch.mean(torch.square(mano_scale[1:] - mano_scale[:-1]))
            total_loss += torch.mean(torch.square(flame_scale[1:] - flame_scale[:-1]))
            total_loss += torch.mean(torch.square(verts_p_face[1:] - verts_p_face[:-1]))
            total_loss += torch.mean(torch.square(verts_p_hand[1:] - verts_p_hand[:-1]))
            ...
            if k % 100 == 0:
                with torch.no_grad():
                    ...
                    depth_images, _ = self.deca.render_hand_head.render_depth(trans_verts, trans_verts_cam)
                    visdict = {
        ...
        del contact_map, contact_bbox, head_contact_idx
        ...
        for i in range(0, verts_p.shape[0], chunk_size):
            ...
            for j in range(shape_images.shape[0]):
                ...
                visdict = {
                    ...
                    # 'landmarks2d': util.tensor_vis_landmarks(visualize_images, trans_landmarks2d.detach(), isScale=True),
        ...
        betas_ori = betas.clone()#.expand(num_img, -1)
        

        # opt_p = torch.optim.Adam(
        #     [betas, global_orient, hand_poses, face_poses, exp, shape],
        #     lr=1e-4)
        # opt_p = torch.optim.Adam(
        #     [betas, global_orient],
        #     lr=1e-4)
        opt_p = torch.optim.Adam(
            [hand_poses, global_orient],
            lr=1e-4)
        ...
        num_epochs = 101
        scheduler_p = CosineAnnealingLR(opt_p, T_max=num_epochs, eta_min=1e-5)
        scheduler_v = CosineAnnealingLR(opt_v, T_max=num_epochs, eta_min=1e-4)
        ...
        dataset = SmallDataset(depth_images_pths, shape[0], exp, face_landmarks, face_poses, betas, global_orient, hand_landmarks, hand_poses, translation_v, hand_mask_pths, mano_scale, head_mask_pths, translation_f, betas_ori, hand_poses_ori, global_orient_ori, weight_hand_lmks, self.scales_all, flame_scale, bz_translation_p)
        batch_size = 80
        data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=False, drop_last=False)
        
        # sliding window dataset
        window_size = batch_size  # batch size
        stride = 60      # 滑动步长，重叠部分=window_size-stride=20
        sliding_dataset = SlidingWindowDataset(dataset, window_size, stride)
        train_loader = DataLoader(sliding_dataset, batch_size=1, shuffle=False)
        ...
        for k in range(num_epochs):
            ...
            #     b_betas_ori = batch['betas_ori']
            #     b_hand_poses_ori = batch['hand_poses_ori']
            #     b_global_orient_ori = batch['global_orient_ori']
            #     b_idx = batch['idx']
            #     b_flame_scale = batch['flame_scale']
            
            for batch in train_loader:
                b_depth_img = batch['depth_img'][0]
                b_shape = batch['shape'][0]
                b_exp = batch['exp'][0]
                b_face_landmarks = batch['face_landmarks'][0]
                b_face_poses = batch['face_poses'][0]
                b_betas = batch['betas'][0]
                b_global_orient = batch['global_orient'][0]
                b_hand_landmarks = batch['hand_landmarks'][0]
                b_hand_poses = batch['hand_poses'][0]
                b_translation_p = batch['translation_p'][0]
                b_translation_v = batch['translation_v'][0]
                b_hand_mask = batch['hand_mask'][0]
                b_mano_scale = batch['mano_scale'][0]
                b_head_mask = batch['head_mask'][0]
                b_translation_f = batch['translation_f'][0]
                b_betas_ori = batch['betas_ori'][0]
                b_hand_poses_ori = batch['hand_poses_ori'][0]
                b_global_orient_ori = batch['global_orient_ori'][0]
                b_idx = batch['idx'][0]
                b_flame_scale = batch['flame_scale'][0]
                ...
                verts_p_face *= b_flame_scale.unsqueeze(1)
                landmarks3d_p_face *= b_flame_scale.unsqueeze(1)
                landmarks2d_p_face *= b_flame_scale.unsqueeze(1)
                ...
                b_trans_landmarks3d_face = projection(landmarks3d_p_face, cam_intrinsics, w2c_p, no_intrinsics=True)
                b_trans_landmarks3d_hand = projection(landmarks3d_p_hand, cam_intrinsics, w2c_p, no_intrinsics=True)
                
                ## landmark loss
                # landmark_loss2 = l2_distance(b_trans_landmarks2d_face[:, :len_landmark_face, :2], b_face_landmarks[:, :len_landmark_face])
                # landmark_loss2 += 4 * l2_distance(b_trans_landmarks2d_hand[:, :len_landmark_hand, :2], b_hand_landmarks[:, :len_landmark_hand])
                landmark_loss2 = l2_distance_v2(b_trans_landmarks2d_hand[:, :len_landmark_hand, :2], b_hand_landmarks[:, :len_landmark_hand])
                landmark_loss2 = landmark_loss2 * 1000 #00.0 #1000
                total_loss = landmark_loss2
                ...
                pred_depth_images_hand, pred_mask_hand = self.deca.render_hand.render_depth(trans_verts_hand, trans_verts_hand_cam, render_mano=True)
                pred_depth_images_hand = pred_depth_images_hand.reshape(-1,512,512)
                pred_mask_hand = pred_mask_hand.reshape(-1,512,512).bool()
                ...
                pred_depth_images_head, pred_mask_head = self.deca.render.render_depth(trans_verts_head, trans_verts_head_cam)
                pred_depth_images_head = pred_depth_images_head.reshape(-1,512,512)
                pred_mask_head = pred_mask_head.reshape(-1,512,512).bool()
                ...
                pred_depth_images, pred_mask = self.deca.render_hand_head.render_depth(trans_verts, trans_verts_cam)
                pred_depth_images = pred_depth_images.reshape(-1,512,512)
                pred_mask = pred_mask.reshape(-1,512,512).bool()
                
                b_hand_mask = b_hand_mask.bool().cuda()
                b_head_mask = b_head_mask.bool().cuda()
                b_depth_img = b_depth_img.cuda()

                # smooth loss
                # smooth_loss = 0
                # smooth_loss += torch.mean(torch.square(translation_v[1:] - translation_v[:-1]))
                # smooth_loss += torch.mean(torch.square(global_orient[1:] - global_orient[:-1]))
                # smooth_loss += torch.mean(torch.square(global_orient - global_orient_ori))
                # smooth_loss += torch.mean(torch.square(hand_poses[1:] - hand_poses[:-1]))
                # smooth_loss += torch.mean(torch.square(hand_poses - hand_poses_ori))
                # # smooth_loss += torch.mean(torch.square(betas[1:] - betas[:-1]))
                # # smooth_loss += torch.mean(torch.square(betas - betas_ori))
                # smooth_loss += torch.mean(torch.square(mano_scale[1:] - mano_scale[:-1]))
                # smooth_loss += torch.mean(torch.square(betas)) * 1e-1
                # # smooth_loss += torch.mean(torch.square(shape)) * 1e-2
                # # smooth_loss += torch.mean(torch.square(exp)) * 1e-2
                ...
                # smooth_loss += torch.mean(torch.square(landmarks3d_p_hand[1:] - landmarks3d_p_hand[:-1]))
                # # smooth_loss += torch.mean(torch.square(verts_p_face[1:] - verts_p_face[:-1]))
                # smooth_loss = smooth_loss * 20 
                # total_loss += smooth_loss
                
                smooth_loss = 0
                smooth_loss += torch.mean(torch.square(b_translation_v[1:] - b_translation_v[:-1]))
                smooth_loss += torch.mean(torch.square(b_global_orient[1:] - b_global_orient[:-1]))
                smooth_loss += torch.mean(torch.square(b_global_orient - b_global_orient_ori))
                smooth_loss += torch.mean(torch.square(b_hand_poses[1:] - b_hand_poses[:-1]))
                smooth_loss += torch.mean(torch.square(b_hand_poses - b_hand_poses_ori))
                smooth_loss += torch.mean(torch.square(b_mano_scale[1:] - b_mano_scale[:-1]))
                smooth_loss += torch.mean(torch.square(verts_p_hand[1:] - verts_p_hand[:-1]))
                smooth_loss += torch.mean(torch.square(landmarks3d_p_hand[1:] - landmarks3d_p_hand[:-1]))
                smooth_loss = smooth_loss * 20 
                total_loss += smooth_loss
                ...
                bbox_mask_head = modify_batched_masks(overlap_mask_hand.reshape(bz,512,512))
                bbox_mask_head = bbox_mask_head & pred_mask_head.bool() & (~pred_mask_hand) & (~b_hand_mask.bool())
                
                depth_ranking_loss = self.depth_ranking_loss(pred_depth_images_hand.reshape(bz,512,512), pred_depth_images_head.reshape(bz,512,512), b_depth_img.reshape(bz,512,512), (pred_mask_hand & b_hand_mask.bool()).reshape(bz,512,512), (pred_mask_head & b_head_mask.bool()).reshape(bz,512,512)) # whole hand and whole head
                depth_ranking_loss = depth_ranking_loss + self.depth_ranking_loss(pred_depth_images_hand.reshape(bz,512,512), pred_depth_images_head.reshape(bz,512,512), b_depth_img.reshape(bz,512,512), overlap_mask_hand.reshape(bz,512,512), bbox_mask_head.reshape(bz,512,512)) # address the overlap area
                depth_ranking_loss = depth_ranking_loss + 10 * self.depth_ranking_loss(pred_depth_images_hand.reshape(bz,512,512), pred_depth_images_hand.reshape(bz,512,512), b_depth_img.reshape(bz,512,512), (pred_mask_hand & b_hand_mask.bool()).reshape(bz,512,512), (pred_mask_hand & b_hand_mask.bool()).reshape(bz,512,512)) # to make sure the depth order inside the hand is correct (global orient)
                depth_ranking_loss = depth_ranking_loss * 10
                ...
                # non_overlap_hand = ~(pred_mask_hand & b_hand_mask)
                # mask_loss += self.cross_entropy_loss(pred_mask_hand.float()[non_overlap_hand], b_hand_mask.float()[non_overlap_hand]) / float(non_overlap_hand.sum()) 
                ...
                contact_loss = contact_loss * 1000
                # total_loss += contact_loss  
                
                # # contact loss
                # contact_loss = bidirectional_contact_loss(verts_p_hand[:,contact_idx,:], verts_p_face[:,flame_face_masks,:])
                # total_loss += contact_loss
                ...
                trans_verts = projection(verts_p[::20], cam_intrinsics, w2c_p[::20])
                shape_images = self.deca.render_hand_head.render_shape(verts_p[::20], trans_verts)
                
                # min_depth = (pred_depth_images.reshape(bz,512,512)*pred_mask).min()
                # max_depth = (pred_depth_images.reshape(bz,512,512)*pred_mask).max()
                min_depth = (pred_depth_images.reshape(bz,512,512)).min()
                max_depth = (pred_depth_images.reshape(bz,512,512)).max()
                pred_depth_images_norm = (pred_depth_images - min_depth) / (max_depth - min_depth)
                
                visdict = {
                        'shape_images': shape_images,
                        # 'pred_depth_images': (predicted_depth.reshape(bz,512,512)*pred_mask)[::20].unsqueeze(1),
                        'pred_depth_images_disp': (pred_depth_images_norm.reshape(bz,512,512)*pred_mask)[::20].unsqueeze(1),
                        'gt_depth_images': b_depth_img[::20].unsqueeze(1),
                        'pred_depth_images_disp_overlap': (pred_depth_images_norm.reshape(bz,512,512)*overlap_mask_hand)[::20].unsqueeze(1),
                        'pred_depth_images_disp_bbox': (pred_depth_images_norm.reshape(bz,512,512)*bbox_mask_head)[::20].unsqueeze(1),
                ...
                del verts_p, trans_verts, shape_images, pred_depth_images, pred_depth_images_hand, pred_depth_images_head#, pred_depth_images_norm, pred_depth_images_hand_norm, pred_depth_images_head_norm
            ...
            # if k % 50 == 0:
                with torch.no_grad():
                    ...
                    def draw_bbox(bbox, image):
                        ...
                        image = image.reshape(512,512,3) * 255
                        image = cv2.rectangle(np.ascontiguousarray(image), (xmin, ymin), (xmax, ymax), box_color, thickness)
                        
                        image = np.array(image/255)#.reshape(512,512,3)
...
if __name__ == '__main__':
    ...
    model = Optimizer(save_folder=args.path)
